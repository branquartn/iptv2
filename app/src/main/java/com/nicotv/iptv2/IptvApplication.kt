...
import com.nicotv.iptv2.data.network.AuthApi
import com.nicotv.iptv2.data.network.CatalogApi
import com.nicotv.iptv2.data.network.NicoTvApi
import com.nicotv.iptv2.data.network.RealtimeClient
import com.nicotv.iptv2.data.network.TmdbApi
import com.nicotv.iptv2.data.repository.MediaRepository
import com.nicotv.iptv2.download.DownloadRepository
import com.nicotv.iptv2.util.FinishedMoviesCache
import com.nicotv.iptv2.util.MovieCastCache
import com.nicotv.iptv2.util.PresenceScreen
import com.nicotv.iptv2.util.SessionManager
...
class IptvApplication : Application() {
    ...
    fun startAppHeartbeat() {
        ...
        appHbJob = appScope.launch {
            while (isActive) {
                if (sessionManager.isLoggedIn() && com.nicotv.iptv2.player.PlayerActivity.current == null) {
    ...
    fun reportScreen(label: String) {
        ...
        if (!sessionManager.isLoggedIn() || com.nicotv.iptv2.player.PlayerActivity.current != null) return
    ...
    private fun handleRemoteCommand(data: org.json.JSONObject) {
        ...
        when (data.optString("cmd")) {
            "pause" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remotePause() }
            "resume" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remoteResume() }
            "seek" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remoteSeek((data.optDouble("value", 0.0) * 1000).toLong()) }
            "volume" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remoteSetVolume(data.optDouble("value", 1.0).toFloat()) }
            "mute" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remoteSetMute(true) }
            "unmute" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remoteSetMute(false) }
            ...
            "audio" -> mainHandler.post {
                ...
                if (idx >= 0) com.nicotv.iptv2.player.PlayerActivity.current?.remoteSetAudioTrack(idx)
            }
            "subtitle" -> mainHandler.post {
                ...
                com.nicotv.iptv2.player.PlayerActivity.current?.remoteSetSubtitleTrack(idx)
            }
            "get_tracks" -> mainHandler.post { com.nicotv.iptv2.player.PlayerActivity.current?.remoteReportTracks() }
            ...
            "nav_back" -> mainHandler.post {
                val player = com.nicotv.iptv2.player.PlayerActivity.current
            ...
            "play" -> {
                ...
                appScope.launch {
                    ...
                    val intent = android.content.Intent(this@IptvApplication, com.nicotv.iptv2.player.PlayerActivity::class.java).apply {
                        addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK)
                        putExtra(com.nicotv.iptv2.player.PlayerActivity.EXTRA_MOVIE_ID, movie.id)
                        putExtra(com.nicotv.iptv2.player.PlayerActivity.EXTRA_STREAM_URL, movie.streamUrl)
                        putExtra(com.nicotv.iptv2.player.PlayerActivity.EXTRA_TITLE, movie.title)
                        putExtra(com.nicotv.iptv2.player.PlayerActivity.EXTRA_RESUME, true)
                    }
