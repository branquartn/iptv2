...
mécanique que `apk2-builder.sh`, **sans AAB** (pas de cible Play Store pour
cette app — sideload uniquement).
...
version installée. **2 derniers APK** conservés dans `/update/` **et** dans
`server/update/` du dépôt (`iptv2-builder.sh` les synchronise et push
automatiquement après un build réussi).

## Process de release

**Répartition (même consigne que NicoTV) :** Claude bumpe la version, commit
et push — **jamais de build**. L'utilisateur (ou le panel) s'occupe du build
et du déploiement.
...
4. Le script publie automatiquement l'APK + `version.json` dans `/update/`
   **et** dans `server/update/` du dépôt (commit + push inclus).
